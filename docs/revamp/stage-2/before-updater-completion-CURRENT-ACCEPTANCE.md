...
| No executable-adjacent legacy auto-migration/fallback | PASS, source/unit and release-migration-matrix.json, three cases | Earlier release artifact tested the unchanged root-selection logic. Native missing-OS-directory fault and stable migration regression are not independently completed. |
| No active stable updater configuration or executable installation path | PASS-SOURCE/config and native rejection | Exhaustive request tracing and cached-manifest UI case not completed. |
...
| All existing routes preserved/open; keyboard at both sizes | PASS: previous 40 pointer/route-size cases, now 40 keyboard activation cases on current package | Every control/live-data workflow is not claimed. |
...
## Pending app-source approval

[UPDATER-DISABLED-UI-REVIEW.md](UPDATER-DISABLED-UI-REVIEW.md): Settings offers inactive Preview updater controls without a disabled-state message. One proposed UI patch is prepared but not applied; source guards remain intact.
...
- [EXIT-STATUS-AND-NATIVE-UI.md](EXIT-STATUS-AND-NATIVE-UI.md): native error-dialog exit 1; 11 real GTK chooser/import/replacement assertions; current Debian/AppImage each pass 23 smoke checks plus resource fallback.
...
Current hashes: evidence/release-artifacts.json (same artifacts as exit-release-artifacts.json). Current reviewable implementation: implementation.patch plus implementation.json. Original source preservation: evidence/upgrade-keyboard-baseline-check.json, 850 files / zero mismatches.
