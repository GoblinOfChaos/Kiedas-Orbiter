...
with open(evidence/'native-picker-equipped-install.log','w') as log:
...
try:
 ...
 xvfb=start([str(bins/'Xvfb'),':'+str(display),'-screen','0','1200x800x24','-nolisten','unix','-listen','tcp','-ac'], 'native-picker-packaged-xvfb.log')
 time.sleep(2)
 if xvfb.poll() is not None:raise RuntimeError('Headless display failed; see native-picker-packaged-xvfb.log')
 ...
 bus=subprocess.Popen([str(bins/'dbus-daemon'),'--nofork','--print-address=1','--config-file='+str(conf)],env=env,stdout=subprocess.PIPE,stderr=open(evidence/'native-picker-packaged-dbus.log','w'),text=True);processes.append(bus)
 import select
 if not select.select([bus.stdout],[],[],5)[0]:raise RuntimeError('Private D-Bus did not start')
 env['DBUS_SESSION_BUS_ADDRESS']=bus.stdout.readline().strip()
 driver=start([str(bins/'WebKitWebDriver'),'--port=17777'],'native-picker-packaged-webdriver.log')
 time.sleep(1)
 ...
 click('Choose source folder');window=x.wait(title);x.focus(window);x.key(ord('l'),0xffe3);x.text(str(source)+'/');x.key(0xff0d);time.sleep(.7)
 if any(w['title']==title for w in x.windows()):x.key(0xff0d);time.sleep(.7)
 ...
 with open(evidence/'native-picker-equipped-remove.log','w') as log:
 ...
 (evidence/'native-picker-packaged-smoke.json').write_text(json.dumps(results,indent=2)+'\n')
