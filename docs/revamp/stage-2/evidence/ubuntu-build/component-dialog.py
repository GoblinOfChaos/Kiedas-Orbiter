...
try:
 ...
 def state(name):
  ...
  check(name+'-viewport-bounds',value['within_viewport'])
  reach=js("const d=document.querySelector('[role=dialog]');return [...d.querySelectorAll('button:not(:disabled),input:not(:disabled)')].map(e=>{e.scrollIntoView({block:'nearest'});const r=e.getBoundingClientRect(),b=d.getBoundingClientRect();return {label:e.innerText||e.parentElement.innerText,visible:r.top>=Math.max(0,b.top)&&r.bottom<=Math.min(innerHeight,b.bottom),scrollTop:d.scrollTop}})")
  value['control_reachability']=reach
  check(name+'-controls-reachable',all(x['visible'] for x in reach))
  value['scroll_metrics']=js("const d=document.querySelector('[role=dialog]');return {clientHeight:d.clientHeight,scrollHeight:d.scrollHeight,overflowY:getComputedStyle(d).overflowY,maxHeight:getComputedStyle(d).maxHeight}")
 ...
 for width,height in [(1200,800),(900,500)]:
  for mode in ['success','error']:
   ...
   check(tag+'-keyboard-focus-visible',js("const r=document.activeElement.getBoundingClientRect();return r.top>=0&&r.bottom<=innerHeight"))
   key('\ue00c');time.sleep(.1)
