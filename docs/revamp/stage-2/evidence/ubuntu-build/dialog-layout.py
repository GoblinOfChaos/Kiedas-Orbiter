...
with open(evidence/'dialog-layout-equipped-install.log','w') as log:
...
try:
 ...
 xvfb=start([str(bins/'Xvfb'),':'+str(display),'-screen','0','1200x800x24','-nolisten','unix','-listen','tcp','-ac'], 'dialog-layout-packaged-xvfb.log')
 time.sleep(2)
 if xvfb.poll() is not None:raise RuntimeError('Headless display failed; see dialog-layout-packaged-xvfb.log')
 ...
 bus=subprocess.Popen([str(bins/'dbus-daemon'),'--nofork','--print-address=1','--config-file='+str(conf)],env=env,stdout=subprocess.PIPE,stderr=open(evidence/'dialog-layout-packaged-dbus.log','w'),text=True);processes.append(bus)
 import select
 if not select.select([bus.stdout],[],[],5)[0]:raise RuntimeError('Private D-Bus did not start')
 env['DBUS_SESSION_BUS_ADDRESS']=bus.stdout.readline().strip()
 driver=start([str(bins/'WebKitWebDriver'),'--port=17777'],'dialog-layout-packaged-webdriver.log')
 time.sleep(1)
 ...
 results['dialog_layout']=[]
 for width,height in [(1200,800),(900,500)]:
  api('POST',prefix+'/window/rect',{'width':width,'height':height});time.sleep(.3)
  click_text('Import a profile copy');time.sleep(.3)
  state=js("const d=document.querySelector('[role=dialog]');const r=d.getBoundingClientRect();return {viewport:[innerWidth,innerHeight],dialog:{top:r.top,bottom:r.bottom,left:r.left,right:r.right,height:r.height},containerOverflow:getComputedStyle(d.parentElement).overflowY,controls:[...d.querySelectorAll('button,input')].map(e=>{const b=e.getBoundingClientRect();return {label:e.innerText||e.parentElement.innerText,disabled:e.matches(':disabled'),top:b.top,bottom:b.bottom}}),active:document.activeElement.innerText}")
  results['dialog_layout'].append({'requested_size':[width,height],'state':state,'fully_within_viewport':state['dialog']['top']>=0 and state['dialog']['bottom']<=state['viewport'][1]})
 ...
 with open(evidence/'dialog-layout-equipped-remove.log','w') as log:
 ...
 (evidence/'dialog-layout-packaged-smoke.json').write_text(json.dumps(results,indent=2)+'\n')
