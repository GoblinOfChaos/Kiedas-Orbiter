...
try:
 ...
 click('Choose source folder');window=x.wait(title);x.focus(window);x.key(0xff50,0xffe9);time.sleep(1);x.key(ord('l'),0xffe3);time.sleep(.3);x.key(ord('a'),0xffe3);x.text(str(source));time.sleep(.5);x.key(0xff0d);time.sleep(3)
 if any(w['title']==title for w in x.windows()):x.key(ord('o'),0xffe9);time.sleep(2)
 if any(w['title']==title for w in x.windows()):results['native_accept_click']=x.accept_click(window);time.sleep(1)
 x.screenshot(evidence/'picker-selection-diagnostic.png')
 ...
 second=scratch/'picker-replacement';(second/'notes').mkdir(parents=True);(second/'map-configs').mkdir();(second/'inventory.json').write_text('{}');(second/'notes/native.md').write_text('Native picker replacement')
 click('Reload Preview');time.sleep(3);click('Import a profile copy');click('Choose source folder')
 window=x.wait(title);x.focus(window);x.key(0xff50,0xffe9);time.sleep(1);x.key(ord('l'),0xffe3);time.sleep(.3);x.key(ord('a'),0xffe3);x.text(str(second));time.sleep(.5);x.key(0xff0d);time.sleep(2)
 if any(w['title']==title for w in x.windows()):results['replacement_accept_click']=x.accept_click(window);time.sleep(1)
 assert js("return document.querySelector('[role=dialog]').innerText.includes(arguments[0])",[str(second)]),'Replacement folder selection failed'
 click('Import selected');time.sleep(.5);results['replacement_rejection']=js("return document.querySelector('[role=status]').innerText")
 results['picker_checks'].append({'name':'replacement_requires_ui_opt_in','pass':'Enable replacement' in results['replacement_rejection']})
 results['picker_checks'].append({'name':'rejected_replacement_keeps_original','pass':(preview/'notes/native.md').read_text()=='Native picker original'})
 el=api('POST',prefix+'/element',{'using':'xpath','value':'//fieldset/label[last()]/input'})['value']['element-6066-11e4-a52e-4f735466cecf'];api('POST',prefix+'/element/'+el+'/click',{})
 click('Import selected');time.sleep(.5);results['replacement_status']=js("return document.querySelector('[role=status]').innerText")
 results['picker_checks'].append({'name':'opted_in_ui_replacement_reports_success','pass':'Imported ' in results['replacement_status']})
 results['picker_checks'].append({'name':'replacement_content_applied','pass':(preview/'notes/native.md').read_text()=='Native picker replacement'})
 results['picker_checks'].append({'name':'original_note_backup_retained','pass':any(p.read_text()=='Native picker original' for p in preview.glob('preview-import-backup-*/notes/native.md'))})
 results['picker_checks'].append({'name':'both_import_sources_unchanged','pass':(source/'notes/native.md').read_text()=='Native picker original' and (second/'notes/native.md').read_text()=='Native picker replacement'})
 x.close()
 api('DELETE',prefix)
