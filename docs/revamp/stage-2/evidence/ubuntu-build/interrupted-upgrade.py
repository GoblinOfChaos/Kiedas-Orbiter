...
for candidate in [package,new_package]:assert subprocess.check_output(['dpkg-deb','-f',str(candidate),'Package'],text=True).strip()=='kieda-s-orbiter-preview'

with open(evidence/'interrupted-upgrade-equipped-install.log','w') as log:
...
try:
 ...
 xvfb=start([str(bins/'Xvfb'),':'+str(display),'-screen','0','1200x800x24','-nolisten','unix','-listen','tcp','-ac'], 'interrupted-upgrade-packaged-xvfb.log')
 time.sleep(2)
 if xvfb.poll() is not None:raise RuntimeError('Headless display failed; see interrupted-upgrade-packaged-xvfb.log')
 ...
 bus=subprocess.Popen([str(bins/'dbus-daemon'),'--nofork','--print-address=1','--config-file='+str(conf)],env=env,stdout=subprocess.PIPE,stderr=open(evidence/'interrupted-upgrade-packaged-dbus.log','w'),text=True);processes.append(bus)
 import select
 if not select.select([bus.stdout],[],[],5)[0]:raise RuntimeError('Private D-Bus did not start')
 env['DBUS_SESSION_BUS_ADDRESS']=bus.stdout.readline().strip()
 driver=start([str(bins/'WebKitWebDriver'),'--port=17777'],'interrupted-upgrade-packaged-webdriver.log')
 time.sleep(1)
 ...
 results['old_package_sha256']=hashlib.sha256(package.read_bytes()).hexdigest()
 with open(evidence/'interrupted-upgrade-install-new.log','w') as log:subprocess.run(['dpkg','-i',str(new_package)],stdout=log,stderr=subprocess.STDOUT,check=True)
 results['new_package_sha256']=hashlib.sha256(new_package.read_bytes()).hexdigest();results['new_binary_sha256']=hashlib.sha256(binary.read_bytes()).hexdigest()
 from native_x11 import X11
 x=X11(env['DISPLAY']);new_app=start([str(binary)],'interrupted-upgrade-newapp.log')
 window=x.wait("Kieda's Orbiter Preview recovery blocked");results['blocked_windows']=x.windows();x.focus(window);x.key(0xff0d);results['new_app_exit']=new_app.wait(timeout=10);x.close()
 results['after_upgrade']=snapshot()
 results['upgrade_checks']={
  'old_crash_left_partial_profile':results['after_crash']=={'a.md':'Replacement a.md','aa.md':'Replacement aa.md','b.md':None},
  'different_installed_payload':results['new_binary_sha256']!=results['binary_sha256'],
  'legacy_partial_upgrade_blocked':results['new_app_exit']==1 and 'Legacy unfinished import' in (evidence/'interrupted-upgrade-newapp.log').read_text(),
  'no_normal_window':not any(w['title']=="Kieda's Orbiter Preview" for w in results['blocked_windows']),
  'partial_profile_not_guessed_at':results['after_upgrade']==results['after_crash'],
  'legacy_backup_remains_unjournaled':not list(preview.glob('preview-import-backup-*/journal.json')),
  'original_backups_retained':any((p/'notes/a.md').read_text()=='Original a.md' and (p/'notes/b.md').read_text()=='Original b.md' for p in preview.glob('preview-import-backup-*')),
 ...
 with open(evidence/'interrupted-upgrade-equipped-remove.log','w') as log:
 ...
 (evidence/'interrupted-upgrade-packaged-smoke.json').write_text(json.dumps(results,indent=2)+'\n')
