...
with open(evidence/'ubuntu-locale-equipped-install.log','w') as log:
...
try:
 ...
 xvfb=start([str(bins/'Xvfb'),':'+str(display),'-screen','0','1200x800x24','-nolisten','unix','-listen','tcp','-ac'], 'ubuntu-locale-packaged-xvfb.log')
 time.sleep(2)
 if xvfb.poll() is not None:raise RuntimeError('Headless display failed; see ubuntu-locale-packaged-xvfb.log')
 ...
 bus=subprocess.Popen([str(bins/'dbus-daemon'),'--nofork','--print-address=1','--config-file='+str(conf)],env=env,stdout=subprocess.PIPE,stderr=open(evidence/'ubuntu-locale-packaged-dbus.log','w'),text=True);processes.append(bus)
 import select
 if not select.select([bus.stdout],[],[],5)[0]:raise RuntimeError('Private D-Bus did not start')
 env['DBUS_SESSION_BUS_ADDRESS']=bus.stdout.readline().strip()
 driver=start([str(bins/'WebKitWebDriver'),'--port=17777'],'ubuntu-locale-packaged-webdriver.log')
 time.sleep(1)
 ...
 ipc('set_setting',{'key':'disclaimer-accepted','value':'true'})
 api('POST',prefix+'/refresh',{});time.sleep(2)
 api('POST',prefix+'/window/rect',{'width':900,'height':500})
 locales=sorted(p.stem for p in (base/'stage2/src/lib/i18n').glob('*.json'))
 results['locale_checks']=[]
 for locale in locales:
  reply=ipc('set_setting',{'key':'uiLocale','value':locale});time.sleep(1)
  state=js("return {labels:[...document.querySelectorAll('nav [id^=preview-group-] button:not([aria-label])')].map(x=>x.innerText.trim()),overflow:document.documentElement.scrollWidth>innerWidth}")
  expected=json.loads((base/'stage2/src/lib/i18n/en.json').read_text())['ui'];expected.update(json.loads((base/'stage2/src/lib/i18n'/ (locale+'.json')).read_text()).get('ui',{}))
  ids=['dashboard','inventory','mastery','mods','cosmetics','collectibles','foundry','relics','relic-planner','prime-resurgence','market','rivens','history','adversaries','maps','wiki','notes','checklist','settings','about']
  expected_labels=[expected.get('nav.'+key) for key in ids]
  results['locale_checks'].append({'locale':locale,'reply':reply,'state':state,'expected_labels':expected_labels,'pass':reply['ok'] and state['labels']==expected_labels and not state['overflow']})
 results['concurrent_settings']=api('POST',prefix+'/execute/async',{'script':"const done=arguments[arguments.length-1];Promise.all(Array.from({length:10},(_,i)=>window.__TAURI_INTERNALS__.invoke('set_setting',{key:'synthetic-concurrent-'+i,value:i}))).then(()=>done(true)).catch(e=>done(String(e)))",'args':[]})['value']
 current=json.loads((preview/'settings.json').read_text());results['concurrent_values_retained']=all(current.get('synthetic-concurrent-'+str(i))==i for i in range(10))
 api('DELETE',prefix)
 ...
 with open(evidence/'ubuntu-locale-equipped-remove.log','w') as log:
 ...
 (evidence/'ubuntu-locale-packaged-smoke.json').write_text(json.dumps(results,indent=2)+'\n')
