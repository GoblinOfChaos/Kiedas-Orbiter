...
with open(root.parent.parent/'docs/revamp/stage-2/evidence/ubuntu-release-bundle.log','w') as log:
 r=subprocess.run(['node','node_modules/@tauri-apps/cli/tauri.js','bundle','--bundles','deb,appimage','--config','src-tauri/tauri.preview.conf.json','--features','preview,tauri/custom-protocol','--no-sign','--ci'],cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT)
