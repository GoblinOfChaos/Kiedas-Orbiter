...
with open(evidence/'recovery-concurrent-equipped-install.log','w') as log:
...
try:
 ...
 xvfb=start([str(bins/'Xvfb'),':'+str(display),'-screen','0','1200x800x24','-nolisten','unix','-listen','tcp','-ac'], 'recovery-concurrent-packaged-xvfb.log')
 time.sleep(2)
 if xvfb.poll() is not None:raise RuntimeError('Headless display failed; see recovery-concurrent-packaged-xvfb.log')
 ...
 bus=subprocess.Popen([str(bins/'dbus-daemon'),'--nofork','--print-address=1','--config-file='+str(conf)],env=env,stdout=subprocess.PIPE,stderr=open(evidence/'recovery-concurrent-packaged-dbus.log','w'),text=True);processes.append(bus)
 import select
 if not select.select([bus.stdout],[],[],5)[0]:raise RuntimeError('Private D-Bus did not start')
 env['DBUS_SESSION_BUS_ADDRESS']=bus.stdout.readline().strip()
 driver=start([str(bins/'WebKitWebDriver'),'--port=17777'],'recovery-concurrent-packaged-webdriver.log')
 time.sleep(1)
 ...
 sources=[]
 for n in ['one','two']:
  source=scratch/n;(source/'notes').mkdir(parents=True);(source/'notes/shared.md').write_text(n);sources.append(str(source))
 result=api('POST',prefix+'/execute/async',{'script':"const done=arguments[arguments.length-1];Promise.all(arguments[0].map(source=>window.__TAURI_INTERNALS__.invoke('import_preview_profile',{source,categories:['notes'],replace:true}).then(value=>({ok:true,value})).catch(error=>({ok:false,error:String(error)})))).then(done)",'args':[sources]})['value']
 results['concurrent_results']=result
 results['concurrent_checks']={'both_submissions_completed':all(x.get('ok') for x in result),'complete_final_content':(preview/'notes/shared.md').read_text() in ['one','two'],'two_committed_transactions':len(list(preview.glob('preview-import-backup-*/committed')))==2,'sources_unchanged':all((scratch/n/'notes/shared.md').read_text()==n for n in ['one','two'])}
 api('DELETE',prefix)
 ...
 with open(evidence/'recovery-concurrent-equipped-remove.log','w') as log:
 ...
 (evidence/'recovery-concurrent-packaged-smoke.json').write_text(json.dumps(results,indent=2)+'\n')
