...
with open(evidence/'recovery-kill-committed-equipped-install.log','w') as log:
...
subprocess.run(['nice','-n','19','gcc','-shared','-fPIC','-o',str(scratch/'crash-fault.so'),str(base/'ubuntu-build/commit-crash-fault.c'),'-ldl'],check=True)
env['LD_PRELOAD']=str(scratch/'crash-fault.so')
env['PREVIEW_TEST_AFTER_COMMIT']='1'
...
try:
 ...
 xvfb=start([str(bins/'Xvfb'),':'+str(display),'-screen','0','1200x800x24','-nolisten','unix','-listen','tcp','-ac'], 'recovery-kill-committed-packaged-xvfb.log')
 time.sleep(2)
 if xvfb.poll() is not None:raise RuntimeError('Headless display failed; see recovery-kill-committed-packaged-xvfb.log')
 ...
 bus=subprocess.Popen([str(bins/'dbus-daemon'),'--nofork','--print-address=1','--config-file='+str(conf)],env=env,stdout=subprocess.PIPE,stderr=open(evidence/'recovery-kill-committed-packaged-dbus.log','w'),text=True);processes.append(bus)
 import select
 if not select.select([bus.stdout],[],[],5)[0]:raise RuntimeError('Private D-Bus did not start')
 env['DBUS_SESSION_BUS_ADDRESS']=bus.stdout.readline().strip()
 driver=start([str(bins/'WebKitWebDriver'),'--port=17777'],'recovery-kill-committed-packaged-webdriver.log')
 time.sleep(1)
 ...
 results['recovery_acceptance']={'committed_files_retained':all(results['after_relaunch'][n]=='Replacement '+n for n in ['a.md','aa.md','b.md'])}
 api('DELETE',prefix)
 ...
 with open(evidence/'recovery-kill-committed-equipped-remove.log','w') as log:
 ...
 (evidence/'recovery-kill-committed-packaged-smoke.json').write_text(json.dumps(results,indent=2)+'\n')
