...
with open(evidence/'recovery-kill-recovery-equipped-install.log','w') as log:
...
subprocess.run(['nice','-n','19','gcc','-shared','-fPIC','-o',str(scratch/'crash-fault.so'),str(base/'ubuntu-build/recovery-crash-fault.c'),'-ldl'],check=True)
env['LD_PRELOAD']=str(scratch/'crash-fault.so')
env['PREVIEW_TEST_CRASH_POINT']='/staged/notes/b.md'
env['PREVIEW_TEST_RECOVERY_KILL']=str(scratch/'recovery-kill-once')
...
try:
 ...
 xvfb=start([str(bins/'Xvfb'),':'+str(display),'-screen','0','1200x800x24','-nolisten','unix','-listen','tcp','-ac'], 'recovery-kill-recovery-packaged-xvfb.log')
 time.sleep(2)
 if xvfb.poll() is not None:raise RuntimeError('Headless display failed; see recovery-kill-recovery-packaged-xvfb.log')
 ...
 bus=subprocess.Popen([str(bins/'dbus-daemon'),'--nofork','--print-address=1','--config-file='+str(conf)],env=env,stdout=subprocess.PIPE,stderr=open(evidence/'recovery-kill-recovery-packaged-dbus.log','w'),text=True);processes.append(bus)
 import select
 if not select.select([bus.stdout],[],[],5)[0]:raise RuntimeError('Private D-Bus did not start')
 env['DBUS_SESSION_BUS_ADDRESS']=bus.stdout.readline().strip()
 driver=start([str(bins/'WebKitWebDriver'),'--port=17777'],'recovery-kill-recovery-packaged-webdriver.log')
 time.sleep(1)
 ...
 try:
  response=api('POST','/session',{'capabilities':{'alwaysMatch':{'webkitgtk:browserOptions':{'binary':str(app)}}}})
  prefix='/session/'+response['value']['sessionId'];time.sleep(1)
  results['recovery_launch_response']=api('GET',prefix+'/title')
 except Exception as ex:results['expected_recovery_session_failure']=str(ex)
 results['recovery_kill_marker']=(scratch/'recovery-kill-once').exists()
 results['mid_recovery']=snapshot()
 # Restart WebDriver too: a browser dying during session creation may leave no session id.
 os.killpg(driver.pid,signal.SIGTERM);driver.wait(timeout=8)
 driver=start([str(bins/'WebKitWebDriver'),'--port=17777'],'recovery-kill-recovery-restarted-webdriver.log');time.sleep(1)
 response=api('POST','/session',{'capabilities':{'alwaysMatch':{'webkitgtk:browserOptions':{'binary':str(app)}}}})
 prefix='/session/'+response['value']['sessionId'];time.sleep(7)
 ...
 results['recovery_acceptance']={'originals_restored':results['after_relaunch']==results['before_crash'],'new_file_removed':results['after_relaunch']['aa.md'] is None,'original_files_present':all(results['after_relaunch'][n] is not None for n in ['a.md','b.md'])}
 api('DELETE',prefix)
 ...
 with open(evidence/'recovery-kill-recovery-equipped-remove.log','w') as log:
 ...
 (evidence/'recovery-kill-recovery-packaged-smoke.json').write_text(json.dumps(results,indent=2)+'\n')
