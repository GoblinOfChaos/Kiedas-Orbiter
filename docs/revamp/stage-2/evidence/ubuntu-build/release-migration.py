...
mode=os.environ.get('PREVIEW_TEST_MODE','empty')
preview_seed=scratch/'xdgdata/kiedas-orbiter-preview/data/user'
if mode!='empty':preview_seed.mkdir(parents=True)
if mode=='settings':(preview_seed/'settings.json').write_text('{"synthetic_existing":true}')
...
binary=base/'ubuntu-build/target/release/kiedas-orbiter'
...
processes=[]; process_groups=set(); logs=[];results={'binary_sha256':hashlib.sha256(binary.read_bytes()).hexdigest(),'private_network_namespace':True,'synthetic_profile':str(scratch),'checks':[],'profile_mode':mode}
...
try:
 ...
 xvfb=start([str(bins/'Xvfb'),':'+str(display),'-screen','0','1200x800x24','-nolisten','unix','-listen','tcp','-ac'], 'release-'+mode+'-native-xvfb.log')
 time.sleep(2)
 ...
 bus=subprocess.Popen([str(bins/'dbus-daemon'),'--nofork','--print-address=1','--config-file='+str(conf)],env=env,stdout=subprocess.PIPE,stderr=open(evidence/('release-'+mode+'-native-dbus.log'),'w'),text=True);processes.append(bus)
 import select
 if not select.select([bus.stdout],[],[],5)[0]:raise RuntimeError('Private D-Bus did not start')
 env['DBUS_SESSION_BUS_ADDRESS']=bus.stdout.readline().strip()
 driver=start([str(bins/'WebKitWebDriver'),'--port=17777'],'release-'+mode+'-native-webdriver.log')
 time.sleep(1)
 ...
 (evidence/('release-'+mode+'-native-smoke.json')).write_text(json.dumps(results,indent=2)+'\n')
