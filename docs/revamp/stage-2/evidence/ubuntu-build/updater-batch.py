...
summary=[]
for script,stem in [('updater-deb-smoke','updater-deb'),('updater-appimage-smoke','updater-appimage')]:
 ...
 for k in ['checks','guard_checks','market_guards','picker_checks','dialog_checks','failure_evidence','native_process_exit','bundle_fallback_check','updater_ui_checks']:
  if k in d:row[k]=d[k]
 summary.append(row);(e/'updater-batch-summary.json').write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps({'script':script,'error':d.get('error'),'exit':d.get('native_process_exit'),'picker':d.get('picker_checks'),'dialog':d.get('dialog_checks')}),flush=True)
