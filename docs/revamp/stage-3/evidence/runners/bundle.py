...
with (e/'inventory-native-bundle.log').open('w') as log:r=subprocess.run(['nice','-n','19','node','node_modules/@tauri-apps/cli/tauri.js','bundle','--verbose','--bundles','deb,appimage','--config','src-tauri/tauri.preview.conf.json','--features','preview,tauri/custom-protocol','--no-sign','--ci'],cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT)
