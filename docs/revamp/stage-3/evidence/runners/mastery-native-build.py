...
with (e/'mastery-native-build.log').open('w') as log:r=subprocess.run(['nice','-n','19','cargo','build','--manifest-path','src-tauri/Cargo.toml','--release','--features','preview,tauri/custom-protocol','--offline','--locked'],cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT)
