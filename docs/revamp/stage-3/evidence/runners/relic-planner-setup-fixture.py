...
SOURCE = CHECKOUT / 'src/screens/RelicPlanner.jsx'
BEFORE = WORK / 'before/RelicPlanner.jsx'
for path in (ROOT / 'AGENTS.md', SOURCE, BEFORE, CHECKOUT / 'src/lib/relicParser.js'):
    if not path.exists():
        raise FileNotFoundError(f'precondition path missing: {path}')

FIXTURE.mkdir(parents=True, exist_ok=True)
shutil.copy2(BEFORE, FIXTURE / 'RelicPlanner.before.jsx')
before_text = (FIXTURE / 'RelicPlanner.before.jsx').read_text()
before_text = before_text.replace("from '../", "from '/var/home/jedwards/kiedas-orbiter/.preview-work/stage2/src/")
(FIXTURE / 'RelicPlanner.before.jsx').write_text(before_text)
...
const mock='/var/home/jedwards/kiedas-orbiter/.preview-work/stage3-relic-planner/fixture/mock.js';
export default defineConfig({
  root:'/var/home/jedwards/kiedas-orbiter/.preview-work/stage3-relic-planner/fixture',
...
export async function invoke(command,args){window.fixture.calls.push({command,args});if(command==='get_icons_path')return '/fixture/icons';return '';}
""")
...
""")
print('Stage 3H fixture created')
