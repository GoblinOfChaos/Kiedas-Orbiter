...
const exportData={ExportRelics:[{uniqueName:'/Lotus/Types/Game/Projections/T1VoidProjectionA1Bronze',rewardManifest:'/Pools/A1',era:'Lith',category:'A1',icon:'/Export/Lith.png',vaulted:false},{uniqueName:'/Lotus/Types/Game/Projections/T3VoidProjectionC3Bronze',rewardManifest:'/Pools/C3',era:'Neo',category:'C3',icon:'/Export/Neo.png',vaulted:false},{uniqueName:'/Lotus/Types/Game/Projections/T4VoidProjectionD4Bronze',rewardManifest:'/Pools/D4',era:'Axi',category:'D4',icon:'/Export/Axi.png',vaulted:true}],ExportRewards:{},ExportItems:{},ExportRecipes:{},dict:{},uniqueNameToName:{},EI:{}};
for(const [idx,relic] of exportData.ExportRelics.entries()){const rs=rewards(['Alpha','Gamma','Delta'][idx]);exportData.ExportRewards[relic.rewardManifest]=rs.map(r=>({type:r.uniqueName,rarity:r.rarity}));for(const r of rs){exportData.ExportItems[r.uniqueName]={name:r.name,primeSellingPrice:r.ducats};exportData.dict[r.uniqueName]=r.name;}}
