...
static const uint8_t key[16] = {76, 69, 79, 45, 65, 76, 69, 67,
                                9,  69, 79, 45, 65, 76, 69, 67};
static const uint8_t iv[16] = {49, 50, 70, 71, 66,  51, 54, 45,
...
[[nodiscard]] static std::string getAccountIdFromArgs(int argc, char *argv[]) {
  ...
  for (int arg_index = 1; arg_index < argc; ++arg_index) {
    ...
    if (arg.find("--account-id=") == 0) {
      ...
      accountId.erase(
          std::remove_if(accountId.begin(), accountId.end(), ::isspace),
          accountId.end());
      return accountId;
    }
    if (arg.find("-a=") == 0) {
...
[[nodiscard]] static std::string getNonceFromArgs(int argc, char *argv[]) {
  ...
  for (int arg_index = 1; arg_index < argc; ++arg_index) {
    ...
    if (arg.find("--nonce=") == 0) {
      nonce = arg.substr(8);
      nonce.erase(std::remove_if(nonce.begin(), nonce.end(), ::isspace),
                  nonce.end());
      return nonce;
    }
    if (arg.find("-n=") == 0) {
...
[[nodiscard]] static std::string getAccountIdFromLastData() {
  ...
  try {
    for (const auto &entry : std::filesystem::directory_iterator(".")) {
      if (entry.is_regular_file()) {
        ...
        if (filename.find("lastData_") == 0 &&
            filename.find(".dat") == filename.length() - 4) {
  ...
  for (const auto &datFile : datFiles) {
    ...
    aes::cbcDecrypt(reinterpret_cast<uint8_t *>(decrypted.data()),
                    decrypted.size(), key, 16, iv);
...
[[nodiscard]] static std::string gruzzleAuthz(const ProcessHandle &mod,
  ...
  for (const auto &ai : mod.getAllocations()) {
    if (auto res = mod.externalScan(ai.range, pattern)) {
      ...
      std::string authz =
          "?accountId=" + std::string(accountId, 24) + "&nonce=";
      char c;
      do {
      ...
      for (int i = 0; i < 200 && sessionCheckPos < ai.range.end(); ++i) {
        ...
        if (sessionIdCheck.length() >= 11 &&
            sessionIdCheck.substr(sessionIdCheck.length() - 11) ==
                "&sessionId=") {
          std::string sessionId;
          do {
            c = mod.externalRead<char>(sessionCheckPos);
            sessionCheckPos = sessionCheckPos.add(1);
          } while ((string::isNumberChar(c) || (c >= 'a' && c <= 'f') ||
                    (c >= 'A' && c <= 'F')) &&
                   (sessionId.push_back(c), true));
          if (!sessionId.empty()) {
  ...
  if (!matches.empty()) {
    std::cout << " Found " << matches.size() << " gruzzled crumbs."
              << std::endl;
    return matches[0];
  }
...
static bool isLikelyLogLine(const char *data, size_t len) {
  ...
  if (data[0] < '0' || data[0] > '9')
    return false;
  // Must contain "EE [Info]", "Sys [Info]", or "Script [Info]" after the
  // timestamp
  const char *log_end = data + len;
  const char *p = data;
  // Skip the initial digits (timestamp)
  while (p < log_end && *p >= '0' && *p <= '9')
    ++p;
  ...
  if (static_cast<size_t>(log_end - p) >= 9) {
    if (std::strncmp(p, "EE [Info]", 9) == 0)
      return true;
    if (std::strncmp(p, "Sys [Info]", 10) == 0)
      return true;
    if (std::strncmp(p, "Script [Info]", 13) == 0)
      return true;
    if (std::strncmp(p, "AI [Info]", 9) == 0)
      return true;
    if (std::strncmp(p, "Net [Info]", 10) == 0)
      return true;
  }
  return false;
}

// Scan a raw memory buffer for EE.log lines and write them to the output
// buffer. Returns the number of bytes written to out.
static size_t extractLogLines(const char *input, size_t input_size, char *out,
                              size_t out_capacity) {
  size_t written = 0;
  size_t line_start = 0;
  for (size_t i = 0; i < input_size && written < out_capacity; ++i) {
    if (input[i] == '\n') {
      ...
      if (line_len > 0 && isLikelyLogLine(input + line_start, line_len)) {
        size_t to_copy =
            (std::min)(line_len + 1, out_capacity - written); // +1 for \n
...
// Score how many valid log lines exist in a 64KB window centered on the match
// position.
static size_t scoreMatchPosition(const ProcessHandle &mod,
                                 const Pointer &match_pos, size_t alloc_size) {
  ...
  Pointer read_base = match_pos.sub(
      (std::min)(HALF, match_pos.as<size_t>())); // don't underflow
  size_t read_size = SAMPLE_SIZE;
  // Clamp to allocation size (approximate - we don't have exact alloc end here,
  // but close enough)
...
// Heuristic: true if the address is in the main exe module range
// (~0x00007FF6... or 0x00007FF...) Heap allocations on x64 Windows are
// typically in the 0x00000... to 0x00007E... range.
...
// Find the best candidate for the EE.log ring buffer by scanning all
// allocations.
static DiscoveredBuffer discoverLogBuffer(const ProcessHandle &mod) {
  const Pattern patterns[] = {
      ...
      Pattern(
          "53 63 72 69 70 74 20 5B 49 6E 66 6F 5D 3A 20"), // "Script [Info]: "
  };
    ...
    for (const auto &pattern : patterns) {
      if (auto res = mod.externalScan(ai.range, pattern)) {
        uintptr_t alloc_base =
            reinterpret_cast<uintptr_t>(ai.range.base.as<void *>());
        ...
        if (adjusted_score > best_score) {
          ...
          uintptr_t win_start =
              (match_addr > alloc_base + half) ? match_addr - half : alloc_base;
          win_start = (std::max)(win_start, alloc_base);
          uintptr_t win_end =
              (std::min)(win_start + MAX_BUFFER, alloc_base + alloc_size);
...
static void getWindowRectMode() {
  ...
  if (ctx.hwnd) {
    ...
    std::cout << r.left << " " << r.top << " " << (r.right - r.left) << " "
  ...
  if (found) {
    ...
    XCloseDisplay(dpy);
    std::cout << pos_x << " " << pos_y << " " << attr.width << " "
              << attr.height << std::endl;
  } else {
    XCloseDisplay(dpy);
    std::cout << "not found" << std::endl;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (!providedNonce.empty()) {
    ...
    if (accountId.empty() || accountId.length() != 24) {
      ...
      std::cout << "  - Provide it via --account-id flag or ACCOUNT_ID "
                   "environment variable"
                << std::endl;
      std::cout << "  - Or run the tool once without --nonce to create a "
                   "lastData.dat file"
                << std::endl;
#if SOUP_WINDOWS
      system("pause > nul");
    ...
    std::cout << "Using provided nonce (--nonce or NONCE environment variable)."
              << std::endl;
  ...
  if (authz.empty() && args.skip_scan) {
    ...
    if (accountId.empty() || accountId.length() != 24) {
      ...
      std::cout << "  - Provide it via --account-id flag or ACCOUNT_ID "
                   "environment variable"
                << std::endl;
      std::cout << "  - Or ensure lastData.dat exists in the working directory"
                << std::endl;
#if SOUP_WINDOWS
      system("pause > nul");
#endif
    ...
    std::cout
        << "Error: --skip-scan requires either --nonce flag or memory scanning."
        << std::endl;
#if SOUP_WINDOWS
    system("pause > nul");
#endif
  ...
  if (jr->isObj()) {
    ...
    const char *nameFields[] = {"playerName", "PlayerName", "alias",
                                "Alias",      "name",       "Name",
                                "username",   "Username"};
  ...
  std::string jsonFilename =
      args.output_file.empty() ? "inventory.json" : args.output_file;
  if (args.output_file.empty() && accountName != "unknown" &&
      !accountName.empty()) {
  ...
  aes::cbcEncrypt(reinterpret_cast<uint8_t *>(inventoryWithAccountId.data()),
                  inventoryWithAccountId.size(), key, 16, iv);
  ...
  std::cout << "Saved to " << datFilename << " (encrypted, contains account ID)"
            << std::endl;
#if SOUP_WINDOWS
  system("pause > nul");
#endif
