...
export async function applyMerges({ dataDir = DEFAULT_DATA, out, cacheDir = DEFAULT_CACHE } = {}) {
  ...
  const [deWarframesRaw, deWeaponsRaw, deRecipesRaw, deResourcesRaw, deManifest] = await Promise.all([
  ...
  const [appWarframes, appWeapons, appRecipes, appResources, appImages] = await Promise.all([
    readApp('ExportWarframes.json'), readApp('ExportWeapons.json'), readApp('ExportRecipes.json'),
    readApp('ExportResources.json'), readApp('ExportImages.json'),
  ])
