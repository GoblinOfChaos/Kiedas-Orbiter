...
import { adaptWeapons, WEAPON_APP_FIELDS } from './adapters/weapons.mjs'
...
const REPORT_PATH = '/tmp/de-shadow/weapons-report.json'
...
function mapRecords(value) {
  ...
  if (value?.ExportWeapons) return mapRecords(value.ExportWeapons)
...
const suffix = provenance.categories.ExportWeapons.suffix.replace(/[^A-Za-z0-9._+-]/g, '_')
const deRaw = JSON.parse(await readFile(join(cacheDir, 'assets', `${suffix}.json`), 'utf8'))
const appRaw = JSON.parse(await readFile(join(appDir, 'ExportWeapons.json'), 'utf8'))
const de = adaptWeapons(deRaw)
const app = mapRecords(appRaw)
const { report: mergeReport } = mergeWeapons(app, de)
...
const report = {
  ...
  mergeReport: {
    ...
    changedFields: Object.fromEntries(
      [...new Set(mergeReport.changed.flatMap((entry) => entry.fields))]
        .sort()
        .map((field) => [field, mergeReport.changed.filter((entry) => entry.fields.includes(field)).length]),
    ),
  },
}
...
console.log('Weapons shadow comparison (DE adapter -> export-plus baseline)')
...
console.log(`Changed fields: ${Object.entries(report.mergeReport.changedFields).map(([field, count]) => `${field}=${count}`).join(', ') || 'none'}`)
