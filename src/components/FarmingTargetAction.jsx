...
import { addTarget, createFarmingTarget, loadFarmingTargets, mutateFarmingTargets, subscribeFarmingTargets } from '../lib/farmingTargets/store.js';
...
export default function FarmingTargetAction({ item, className = '' }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeFarmingTargets(setStore);
    loadFarmingTargets().catch(() => {});
    return unsubscribe;
  }, []);
  ...
  const add = async (event) => {
    ...
    if (!store || store.readOnlyCorrupt || exists || saving) return;
    setSaving(true);
    try {
      await mutateFarmingTargets((current) => addTarget(current, createFarmingTarget({ uniqueName, name: item.name, image: item.image, category: item.category })));
