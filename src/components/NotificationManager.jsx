  ...
  return (
    <div>
      <div className="space-y-2">
        ...
        <p className="text-sm text-kronos-dim text-center py-6">{t('ui.notif_mgr.not_configured')}</p>
        }
      ...
      <div className="mt-8 pt-4 border-t border-white/5">
        <button
          ...
          <History size={16} />{t('ui.notif_mgr.session_log')}
          {notificationHistory?.length > 0 && <span className="text-kronos-dim">({notificationHistory.length})</span>}
        </button>
        
        <Modal isOpen={showLog} onClose={() => setShowLog(false)} title={t('ui.notif_mgr.session_log')} maxWidth="max-w-md">
          <div className="space-y-2">
            {!notificationHistory || notificationHistory.length === 0 ?
            <p className="text-sm text-kronos-dim italic text-center py-4">{t('ui.notif_mgr.session_empty')}</p> :
            ...
            <div key={idx} className="flex gap-3 items-start p-3 bg-kronos-panel/20 rounded-lg border border-white/5">
              ...
              <div className="w-8 h-8 bg-black/20 rounded border border-white/5 flex items-center justify-center shrink-0">
                      <span className="text-[10px] text-white/30 font-bold uppercase">{t('ui.notif_mgr.msg')}</span>
                    </div>
              }
