...
export default function Maps() {
  ...
  return (
    <PageLayout title="Maps">
          ...
          <div className="flex-1 min-h-0 relative">
            <Card className="glass-panel rounded-lg overflow-hidden p-0 relative min-h-0 bg-black/40 h-full w-full">
              ...
              <button onClick={resetTransform}
                className="absolute bottom-4 right-4 z-10 bg-kronos-bg/80 backdrop-blur px-3 py-1.5 rounded-md text-sm text-kronos-dim font-bold shadow-lg border border-white/5 hover:text-kronos-text transition-colors">
                Reset view
              </button>
              ...
              {selectedMarker && (
                <div data-float-panel="true"
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-sm font-bold text-kronos-text uppercase tracking-wider">Marker Editor</span>
                  ...
                  <div className="flex flex-col gap-3">
                    <div>
                      <label className="text-[11px] font-black text-kronos-accent uppercase tracking-widest px-1">Label</label>
                      <input
                    ...
                    <div>
                      <label className="text-[11px] font-black text-kronos-accent uppercase tracking-widest px-1">Color & Icon</label>
                    ...
                    <div className="pt-2 border-t border-white/10">
                      <label className="text-[11px] font-black text-kronos-accent uppercase tracking-widest px-1">Connections</label>
                      <div className="flex flex-col gap-1 mt-1 max-h-32 overflow-y-auto custom-scrollbar pr-1">
                        ...
                        {configsForCurrentMap.find(c => c.id === selectedMarker.configId)?.markers.length <= 1 && (
                          <div className="text-xs text-kronos-dim py-1 px-1">No other markers to connect to.</div>
                        )}
                      </div>
                    </div>
                    <div>
                      <label className="text-[11px] font-black text-kronos-accent uppercase tracking-widest px-1">Notes</label>
                      <textarea
            ...
            <div className="w-80 flex-shrink-0 glass-panel rounded-lg p-4 flex flex-col gap-3 overflow-y-auto custom-scrollbar" style={{ maxHeight: 'calc(100vh - 200px)' }}>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => invoke('open_map_configs_folder')}
                    className="p-1.5 rounded-lg bg-kronos-bg/80 backdrop-blur text-kronos-dim hover:text-kronos-text border border-white/5 transition-colors"
                    title="Open map configs folder">
              ...
              {configsForCurrentMap.length === 0 && (
                <p className="text-sm text-kronos-dim text-center py-6">No configurations yet.</p>
              )}
                  ...
                  return (
                    <div key={config.id}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-[11px] text-kronos-dim mt-0.5 font-medium">
                            ...
                            {config.paths.length > 0 && ` · ${config.paths.length} path${config.paths.length !== 1 ? 's' : ''}`}
                          </div>
                        </div>
                      ...
                      <div className="flex items-center gap-2 mt-2 pt-2 border-t border-white/5">
                        <button
                          onClick={() => {
                            if (isAdding) { setMode('view'); setPendingConfigId(null) }
                            else { setMode('addMarker'); setPendingConfigId(config.id); setSelectedMarker(null); closeMarkerPanel() }
                          }}
      ...
      {configToEdit && (
        <Modal isOpen={true} onClose={() => setConfigToEdit(null)} title="New Configuration" maxWidth="max-w-sm">
          <div className="flex flex-col gap-4">
            <div>
              <label className="text-xs font-black text-kronos-accent uppercase tracking-widest px-1">Name</label>
            ...
            <div>
              <label className="text-xs font-black text-kronos-accent uppercase tracking-widest px-1">Description</label>
            ...
            <div className="flex gap-2 pt-2 border-t border-white/5">
              <Button variant="ghost" className="flex-1 text-xs" onClick={() => setConfigToEdit(null)}>Cancel</Button>
              <Button variant="primary" className="flex-1 text-xs"
                onClick={() => { addConfig(configToEdit.name, configToEdit.description); setConfigToEdit(null) }}>
                Create
              </Button>
            </div>
          </div>
      ...
      {deleteConfirm && (
        <Modal isOpen={true} onClose={() => setDeleteConfirm(null)} title="Confirm Delete" maxWidth="max-w-sm">
          <div className="flex flex-col gap-4">
            <p className="text-sm text-kronos-dim">
              Delete <span className="text-kronos-text font-bold">{deleteConfirm.name}</span>?
      ...
      {contextMenu && (
        ...
        >
          ...
          >
            ...
            >
              <MapPin size={14} className="text-kronos-accent" />
              Add Marker Here
            </button>
            <div className="px-3 py-1 text-[10px] text-kronos-dim uppercase tracking-wider border-t border-white/5 mt-1 pt-2">
              To: {configsForCurrentMap.find(c => c.id === contextMenu.configId)?.name || 'Config'}
            </div>
          </div>
        </div>
      )}
