...
export default function Mastery() {
  ...
  // Recomputes a dedup/reduce over the full inventory for ~19 categories -
  // expensive enough that leaving it as a plain const (recomputed on every
  // render, including trivial UI state changes like closing the detail
  // modal) caused a noticeable freeze before the UI responded. Memoized so
  // it only reruns when the underlying inventory data actually changes.
              ...
              <div className="relative z-10 flex-1 p-10 flex flex-col items-start text-left">
                <div className="flex items-center gap-12 mb-8">
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    <div className="text-kronos-accent font-black text-sm">{t('ui.inventory.ready_status')}</div>
                  ...
                  <div className="relative">
                    ...
                    <p className="text-xs text-kronos-accent uppercase font-black mt-2 tracking-[0.2em]">{t('mastery.next')}{nextRank > 30 ? `LR${nextRank - 30}` : `MR${nextRank}`}</p>
                  </div>
                </div>

                <h2 className="text-4xl font-black text-kronos-text uppercase tracking-tighter mb-2 max-w-lg leading-none">{t('mastery.rank_up_available')}

              </h2>
                <p className="text-xl text-kronos-accent font-bold italic mb-6">
                  {t('mastery.advance_to', { title: nextTitle })}
                </p>

                <div className="text-sm text-kronos-dim max-w-sm leading-relaxed border-t border-kronos-accent/10 pt-6">
                  {t('mastery.rank_up_test')}
                </div>
              </div>
            ...
            className="bg-gradient-to-br from-kronos-accent/10 via-transparent to-transparent p-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
                ...
                <div className="flex flex-col md:items-end">
                  <div className="text-[10px] text-kronos-accent uppercase font-black tracking-[0.3em] mb-2 opacity-80">{t('mastery.next_rank')}</div>
          ...
          onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 border-b border-kronos-panel flex items-center justify-between bg-kronos-panel/20">
              ...
              <label className="flex items-center gap-2 cursor-pointer group">
                  ...
                  onClick={() => setHideNonMastery(!hideNonMastery)}>
                    ...
                    <span className="text-[10px] font-black uppercase text-kronos-dim group-hover:text-kronos-accent tracking-widest select-none">{t('mastery.hide_non_mastery')}
            ...
            (() => {
              ...
              if (!nodes.length) return (
                <div className="text-center py-8 text-kronos-dim italic">{t('mastery.no_mastery_nodes')}
                      ...
                      return (
                        <div key={node.tag} className={`flex items-center justify-between p-2 rounded bg-kronos-panel/10 border border-kronos-panel/20 mb-1 ${completed ? 'opacity-50' : ''}`}>
                            ...
                            <Circle size={14} className="text-kronos-dim flex-shrink-0" />}
                                ...
                                {node.isJunction &&
                            <span className="text-[8px] bg-kronos-accent/20 text-kronos-accent px-1 rounded font-black uppercase tracking-tighter">{t('mastery.junction')}
            ...
            <div className="text-center py-8 text-kronos-dim italic">{t('mastery.no_items_found')}
