...
export default function Relics() {
  const { t } = useUi()
  const { inventoryData, exportData, isInventoryLoading, allPrices, isPriceLoading, priceFetchProgress, dropIndex, recipeResultIndex, marketIndex } = useMonitoring();
  ...
  const openItem = useMemo(() => {
    ...
    if (!item) return null;
    const info = getAcquisitionInfo(item.unique_name, item.name, dropIndex, acquisitionOverrides, recipeResultIndex, marketIndex);
  ...
  }, [openKey, acquisitionDataReady, grouped, dropIndex, acquisitionOverrides, recipeResultIndex, marketIndex]);
